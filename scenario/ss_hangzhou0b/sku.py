"""SKU definitions and pallet sizes for the ss_hangzhou0b scenario.
...
if sku_path.exists():
    ...
    for i in range(1, N + 1):
        ...
        sku_id = f"sauce_{i}"
        SKUS[sku_id] = SKU(
            id=sku_id, name=f"酱包{i}",
            bom={f"smallpack_s_{i}": 1, f"meat_{_sauce_meat[idx]}": 2,
                 f"oil_{_sauce_oil[idx]}": 2, f"vegetable_{_sauce_veg[idx]}": 2},
            bom_speed=10, pallet_size=100,
        )
    ...
    for i in range(1, N + 1):
        ...
        sku_id = f"powder_{i}"
        SKUS[sku_id] = SKU(
            id=sku_id, name=f"粉包{i}",
            bom={f"smallpack_p_{i}": 1, f"original_powder_{a}": 1,
                 f"original_powder_{b}": 1, f"original_powder_{c}": 1},
            bom_speed=10, pallet_size=100,
        )
    ...
    for i in range(1, N + 1):
        ...
        sku_id = f"veg_{i}"
        SKUS[sku_id] = SKU(
            id=sku_id, name=f"菜包{i}",
            bom={f"smallpack_v_{i}": 1, f"dry_veg_{a}": 1, f"dry_veg_{b}": 1},
            bom_speed=100, pallet_size=100,
        )
    ...
    for i in range(1, N + 1):
        ...
        if i % 2 == 0:
            ...
            inputs["pack"] = 1
        SKUS[sku_id] = SKU(
            id=sku_id, name=f"成品面{i}",
            bom=inputs, bom_speed=5, pallet_size=50,
        )